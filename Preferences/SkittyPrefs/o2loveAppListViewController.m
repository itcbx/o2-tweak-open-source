...
#import "o2loveAppListViewController.h"

#define PREFS_PATH @"/var/mobile/Library/Preferences/xc.lzsxcl.o2.o2applove.plist"
#define BUNDLE_ID @"xc.lzsxcl.o2"

o2loveAppListViewController *controller;
BOOL checkAllFlag;
...
@implementation o2loveAppListViewController
...
- (id)init {
	...
	if (self) {
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:PREFS_PATH]) {
			apps = [prefs objectForKey:@"favorites"];//favorites
		} else {
			apps = @[];
		}
		...
		self.searchController.searchBar.placeholder = @"搜索 *可多选";
...
- (void)checkAll {
	...
	NSString *msg = checkAllFlag ? 
		@"貌似太多了，我希望你只是用作反选的清除" : 
		@"是否清空喜爱的应用";
	UIAlertController* alert = [UIAlertController alertControllerWithTitle: @"全部选择"
								   								   message: msg
...
- (void)updatePreferencesAppList {
	NSDictionary *preferencesDict = @{ @"favorites": self.preferencesAppList };
